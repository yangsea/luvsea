...
public class KafkaUtils {
	...
	public static void sendMsgToKafka(IpranAlarm msg) {
		producer.send(new ProducerRecord<String, String>(Constants.TOPIC_NAME,
				String.valueOf(new Date().getTime()), JSON.toJSONString(msg)),new Callback() {
...
//	}
	...
	public static void getMsgFromKafka() {
		...
		System.out.println("come in get msg");
		while (true) {
			ConsumerRecords<String, String> records = KafkaUtils
					.getKafkaConsumer().poll(100);
			for (ConsumerRecord  consumerRecord : records) {
				System.out.println("offset是："+consumerRecord.offset());
			}
			if (records.count() > 0) {
				System.out.println("主题中数量为"+records.count());
				for (ConsumerRecord<String, String> record : records) {
					JSONObject jsonAlarmMsg = JSON.parseObject(record.value());
					IpranAlarm alarmMsg = JSONObject.toJavaObject(jsonAlarmMsg,
							IpranAlarm.class);
//					LOGGER.debug("从kafka接收到的消息是：" + alarmMsg.toString());
